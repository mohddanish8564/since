	...
	if($indicator3Quarter_Data[0]['A']=='' || $indicator3Quarter_Data[0]['A']==null){
		$totalQuarterVal=0;
			...
			<!--   indicator 4 -->
				...
				$indicatorSex_male="(SELECT count(distinct jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
				
				
				$sex='Female';
				$indicatorSex_female="(SELECT count(distinct jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		<?php 
												 				...
												 				/* For Age OverAll */
				$wherecolumn='Youth (Potential Migrant)';
				$indicator1_youth="(SELECT count(distinct jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
			  $wherecolumn='Refugees';				
		   $indicator1_women="(SELECT count(distinct jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
				 $wherecolumn='Returnee';				
		   $indicator1_returnee="(SELECT count(distinct jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
			...
			//	$totalvalue_indicator1=$totalvalue_indicator1+$A+$B+$C;
				...
				$indicatorSQuarter_maleSql="SELECT count(distinct jp.`beneficiary_id`) as mcount  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' $whereBetweenQuarter";
