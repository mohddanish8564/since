...
$valuechainSql="select MVM.name, CASE WHEN mcount is null THEN 0 ELSE mcount END as tcount
from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,count(distinct sme.agr_decent_work_principle) as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name  where sme.`region` like '%$regionName%' and sme.agr_decent_work_principle=true $whereQuarter group by vm.name) as TT  ON TT.name=MVM.NAME where MVM.name in ('Agri-Business','Construction','Leather','Metal Works','Textile') order by MVM.name asc";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		$valuechainQuarterSql="SELECT count(distinct sme.agr_decent_work_principle) as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name  where sme.`region` like '%$regionName%' and sme.agr_decent_work_principle=true $whereBetweenQuarter ";
