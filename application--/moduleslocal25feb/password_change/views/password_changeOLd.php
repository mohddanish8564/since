		...
		if($this->session->userdata('username')!=null)
		  {	
			...
			$template='logo_change';
			 ...
			 $field_arry1=array('user_id'=>$this->session->userdata('user_id'));
			 $data['logoData']=$this->Allfunction->getArray('*','users',$field_arry1);
	  ...
	  $nm1="logo"; $newfilename=""; $imgerror=''; 
	       ...
	       {          $arChk= array('image/png','image/jpg', 'image/jpeg' );
		     if(   in_array(   strtolower($_FILES[$nm1]['type']) , $arChk  )   ) 
				  {    $imgerror='';   
				      ...
				      {  $temp = explode(".", $_FILES[$nm1]["name"]);
					      ...
					      move_uploaded_file($_FILES[$nm1]['tmp_name'], './uploads/logo/'.$newfilename); 
			...
			'logo' =>$newfilename,			
			 ...
			 $field_arry=array('id'=>$this->session->userdata('user_id'));
		      $password_update= $this->Allfunction->tblUpdationByArray($field_arry,$up_data,'companydetails');
			  if ($password_update)
			  {
				$this->session->set_flashdata('textS', 'Logo Changed Successfully');
