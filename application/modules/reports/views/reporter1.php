...
<!-- Indicator2 -->
		         ...
		         <th colspan="2">Age</th> 
					...
					$maincolumn='(count(distinct benetrain.beneficiary_id))';
												 				...
												 				/* For Age OverAll */
				$sex='Male';   
				$indicatorSex_male="(SELECT $maincolumn  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
				

				
				
				$sex='Female';
				$indicatorSex_female="(SELECT $maincolumn  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
				
				 $indicator1_sex_sql="select ".$indicatorSex_male." as male,".$indicatorSex_female." as female FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' group by benetrain.`Region`";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		<?php 
				...
				$indicator1_youth="(SELECT $maincolumn  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
			  $wherecolumn='Women 15-35 (Potential Migrant)';				
				$indicator1_women="(SELECT $maincolumn  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
				 $wherecolumn='Returnee';				
				$indicator1_returnee="(SELECT $maincolumn  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				 $indicator1_targetgrp_sql="select ".$indicator1_youth." as A,".$indicator1_women." as B,".$indicator1_returnee." as C  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%' group by benetrain.`Region`";
		...
		if(	$selectedquarter!='All'){
			 $whereBetweenQuarter=findquaretrBetweenFormat('benetrain.end_date',$selectedyear,$selectedquarter);  // Between Quarter
				...
				$indicator2Er1QuarterSql="SELECT $maincolumn as mcount  FROM `xls_import_beneficiary_trainings` as benetrain inner join xls_import_beneficiaries as bene on bene.beneficiary_id=benetrain.beneficiary_id where benetrain.Is_Training_completed='true' and benetrain.`Region` like '%$regionName%'  $whereBetweenQuarter";
	...
	if($indicator3Quarter_Data[0]['A']=='' || $indicator3Quarter_Data[0]['A']==null){
		$totalQuarterVal=0;
			...
			<!--   indicator 4 -->
				...
				$indicatorSex_male="(SELECT count(jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
				
				
				$sex='Female';
				$indicatorSex_female="(SELECT count(jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.sex='$sex' $whereQuarter)";
				
				
				 $indicator1_sex_sql="select ".$indicatorSex_male." as male,".$indicatorSex_female." as female FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%'    group by jp.`Region`";
...
$valuechainSql="select MVM.name, CASE WHEN mcount is null THEN 0 ELSE mcount END as tcount
from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,count(jp.beneficiary_id) as mcount FROM  valuechain_master as vm left join `xls_import_beneficiaries` as bene on bene.`value_chain`=vm.name left join `xls_import_job_placement` as jp on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' $whereQuarter group by vm.name) aS TT  ON TT.name=MVM.NAME order by MVM.name asc";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		<?php 
												 				...
												 				/* For Age OverAll */
				$wherecolumn='Youth (Potential Migrant)';
				$indicator1_youth="(SELECT count(jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
			  $wherecolumn='Women 15-35 (Potential Migrant)';				
		   $indicator1_women="(SELECT count(jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				
				 $wherecolumn='Returnee';				
		   $indicator1_returnee="(SELECT count(jp.`beneficiary_id`)  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' and bene.`target_group`='$wherecolumn' $whereQuarter)";
				
				 $indicator1_targetgrp_sql="select ".$indicator1_youth." as A,".$indicator1_women." as B,".$indicator1_returnee." as C  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' group by jp.`Region`";
			...
			//	$totalvalue_indicator1=$totalvalue_indicator1+$A+$B+$C;
				...
				$indicatorSQuarter_maleSql="SELECT count(jp.`beneficiary_id`) as mcount  FROM `xls_import_job_placement` as jp inner join xls_import_beneficiaries as bene on bene.beneficiary_id=jp.beneficiary_id where jp.placement_type='Apprenticeship' and jp.`Region` like '%$regionName%' $whereBetweenQuarter";
