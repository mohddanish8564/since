...
// die;
	...
	//			 echo $indicator2Er1QuarterSql.'<br>';
				 ...
				 $indicator3_Sql="select $maincolumn as A from xls_import_pp where (TVET_name!='' and TVET_name is not null) and `Region` like '%$regionName%'  $whereQuarter  group by `Region`";
...
// die;
	// $indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
				 ...
				 $indicator3Quarter_Sql="select $maincolumn as A from xls_import_pp where (TVET_name!='' and TVET_name is not null) and `Region` like '%$regionName%'  $whereBetweenQuarter";
