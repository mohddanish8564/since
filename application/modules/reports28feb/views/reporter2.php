   ...
   <!--Indicator 1 -->			
            ...
            <th>Quarter</th>
			       ...
			       if(	$selectedquarter!='All'){							
													  $whereBetweenQuarter=findquaretrBetweenFormat('sme.SME_Support_date',$selectedyear,$selectedquarter);  // Between Quarter
															$whereQuarter=findquaretrFormat('sme.SME_Support_date',$selectedyear,$selectedquarter);
										}
										
										else{
											
											$whereQuarter="and Year(sme.SME_Support_date)<='$selectedyear'";
...
from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,count(distinct sme.name) as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name  where sme.`region` like '%$regionName%' $whereQuarter group by vm.name) as TT  ON TT.name=MVM.NAME where MVM.name in ('Agri-Business','Construction','Leather','Metal Works','Textile') order by MVM.name asc";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		$valuechainQuarterSql="SELECT count(distinct sme.name) as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name  where sme.`region` like '%$regionName%' $whereBetweenQuarter ";
...
// die;
   ...
   <!--Indicator 2 -->			
			       ...
			       if(	$selectedquarter!='All'){							
									...
									$valuechainSql="select MVM.name, CASE WHEN mcount is null THEN 0 ELSE mcount END as tcount
									from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,$maincolumn as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name  where sme.`region` like '%$regionName%' group by vm.name) as TT  ON TT.name=MVM.NAME where MVM.name in ('Agri-Business','Construction','Leather','Metal Works','Textile') order by MVM.name asc";
...
// die;
	...
	?>
											...
											<td><?php echo round($totalvalue_indicator1,2); ?></td>
											<td><?php echo $indicator1achiveper; ?>%</td>
											<td><?php echo findpercent($totalvalue_indicator1,$indicator1achiveper); ?></td>
   ...
   <!--Indicator 3 -->			
            ...
            <th>Quarter</th>
			       ...
			       if(	$selectedquarter!='All'){							
													  $whereBetweenQuarter=findquaretrBetweenFormat('pp.agr_date',$selectedyear,$selectedquarter);  // Between Quarter
															$whereQuarter=findquaretrFormat('pp.agr_date',$selectedyear,$selectedquarter);
										}
										
										else{
											
											$whereQuarter="and Year(pp.agr_date)<='$selectedyear'";
											$whereBetweenQuarter=$whereQuarter;
										}
...
$valuechainSql="select MVM.name, CASE WHEN mcount is null THEN 0 ELSE mcount END as tcount
from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,count(distinct pp.agr_date) as mcount FROM  valuechain_master as vm left join `xls_import_pp` as pp on pp.`value_Chain`=vm.name  where pp.`Region` like '%$regionName%' $whereQuarter group by vm.name) as TT  ON TT.name=MVM.NAME where MVM.name in ('Agri-Business','Construction','Leather','Metal Works','Textile') order by MVM.name asc";
...
// die;
	$indicator1_valuechain= $this->Allfunction->runQueryGet($valuechainSql);
		...
		$valuechainQuarterSql="SELECT count(distinct pp.agr_date) as mcount FROM  valuechain_master as vm left join `xls_import_pp` as pp on pp.`value_Chain`=vm.name  where pp.`Region` like '%$regionName%' $whereBetweenQuarter ";
...
$valuechainSql="select MVM.name, CASE WHEN mcount is null THEN 0 ELSE mcount END as tcount
from valuechain_master AS MVM LEFT JOIN (SELECT vm.name,count(distinct sme.agr_decent_work_principle) as mcount FROM  valuechain_master as vm left join `xls_import_smecompanies` as sme on sme.`value_chain`=vm.name inner join xls_import_sme_support as smesupp on smesupp.sme_name=sme.enterprise_id  where sme.`region` like '%$regionName%' and smesupp.support_type='Subcontracting arrangement' $whereQuarter group by vm.name) as TT  ON TT.name=MVM.NAME where MVM.name in ('Agri-Business','Construction','Leather','Metal Works','Textile') order by MVM.name asc";
